...
impl Handshake {
    ...
    pub fn read<Reader>(stream: &mut Reader) -> Result<Handshake>
    where
        Reader: Read + std::marker::Unpin,
    {
        let mut protocol_len_bytes = [0; 1];
        stream.read_exact(&mut protocol_len_bytes)?;
...
mod tests {
    ...
    fn test_(msg: Handshake, bytes: &[u8]) {
        {
...

        {
            ...
            let decoded_msg = Handshake::read(&mut cursor).unwrap();
